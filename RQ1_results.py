...
tokenizer = ["bert", "bert", "roberta", "roberta", "roberta", "roberta", "roberta", "roberta"]
            ...
            for project in projects:
                ...
                for index, row in df_results.iterrows():
                    ...
                    if row['prediction'] == "empty" or row['level_prediction'] == "":
                        ind_results_file = f"results/{llm}/{project}/{shot}/output_finetuned_validate_{project}_{llm}_{shot}_{row['index']}.json"
                ...
                df_results[['index', 'project', 'log_level', 'level_prediction']].to_json(f"results/results_{llm}_{project}_{shot}.json", orient="records", indent=2)
    ...
    for llm in pretrainedmodels:
        for shot in shots:
            for project in projects:
                ...
                result_file = f"results/output_{project}_{tokenizer}_{llm}_{shot}.txt"
...
def calculate_metrics_llm_shot_project():
    ...
    for index, llm in enumerate(llms):
        for shot in shots:
            for project in projects:
                ...
                df = pd.read_json(f"results/results_{llm}_{project}_{shot}.json")
    ...
    output.to_json("log_level_results.json", orient="records", indent=2)
