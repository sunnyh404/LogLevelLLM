...
arguments = "_".join(["finetuned", split_type, project, base_model, str(test_size), file_index, "pm"])

output_file = f"results/{base_model}/{project}/{test_size}/output_{arguments}.json"


import pandas as pd
df = pd.read_json(f"logs/{project}_logs_{split_type}_0.6_pm.json")
...
LORA_WEIGHTS = f"saved_model/{base_model}/{project}_{test_size}_pm"


if str(test_size) == "0.5":
    if ft_project == "":
        LORA_WEIGHTS = f"saved_model/{base_model}/{project}_0.5_pm"
    else:
        LORA_WEIGHTS = f"saved_model/{base_model}/{ft_project}_5_pm"
...
if str(test_size) == "5":
    ...
    for  index, row in df_5shot.iterrows():
        shot5 = shot5 + '### Input: The previous method is """' + row['previous_code'] + '""", the source code is """' + row['code'] + '""", and the log message is ' + row['constant'] + '. \n\n### Response:The log level is ' + row['log_level'] + '. \n'
...
for row_index, row in tqdm(df.iterrows(), total=len(df)):
    ...
    template = 'The previous method is """' + row['previous_code'] + '""", the source code is """' + row['code'] + '""", and the log message is ' + row['constant'] + '.'
