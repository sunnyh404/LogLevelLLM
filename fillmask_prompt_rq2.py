...
for project in projects:
    ...
    arguments = "_".join([project, model_name.replace("/", ""), os.path.basename(pretrainedmodel_path), str(test_size)]) + pm
    output_file = f"results/output_{arguments}.txt"
    ...
    with open(output_file, 'w') as f:    
        ...
        if str(test_size) in ("0", "5"):
            ...
            if str(test_size) == "0.5":
                ...
                # train_path = os.path.join(main_path, "logs", project + "_logs_train_" + str(test_size) + ".json")
                train_path = f"logs/{project}_instructions_logs_train_{str(test_size)}{pm}.json"
        ...
        occurrences = train_dataset['log_label'].value_counts()
        ...
        if str(test_size) == "5":
            ...
            # Calculate the lengths of the code strings
            df_5shot['code_length'] = df_5shot['code'].apply(len)

            # Sort by the length of the code, then take the two shortest
            df_5shot = df_5shot.nsmallest(3, 'code_length')
