...
#RQ3
def get_textgen_results_u(suffix):
            ...
            for project in projects:
                ...
                for index, row in df_results.iterrows():
                    ...
                    if row['prediction'] == "empty" or row['level_prediction'] == "":
                        ind_results_file = f"results/{llm}/{project}/{shot}/output_finetuned_validate_{project}_{llm}_{shot}_{row['index']}{suffix}.json"
                ...
                df_results[['index', 'project', 'log_level', 'level_prediction']].to_json(f"results/results_{llm}_{project}_{shot}{suffix}.json", orient="records", indent=2)




def get_fillmask_results_u(suffix):
    ...
    for llm in pretrainedmodels:
        for shot in shots:
            for project in projects:
                ...
                result_file = f"results/output_{project}_{tokenizer}_{llm}_{shot}{suffix}.txt"
...
def calculate_metrics_llm_shot_project_u(suffix):
    ...
    for index, llm in enumerate(llms):
        for shot in shots:
            for project in projects:
                ...
                df = pd.read_json(f"results/results_{llm}_{project}_{shot}{suffix}.json")
    ...
    output.to_json(f"log_level_results{suffix}.json", orient="records", indent=2)



def calculate_metrics_llm_shot_project_x(suffix):
        ...
        for shot in shots:
            for project in projects:
                ...
                for ft_project in ft_projects:
                    ...
                    df = pd.read_json(f"results/results_{llm}_{project}_{ft_project}_{shot}{suffix}.json")
...
def get_textgen_results_x(suffix):
            ...
            for project in projects:
                ...
                for ft_project in ft_projects:
                    ...
                    for index, row in df_results.iterrows():
                        ...
                        if row['prediction'] == "empty" or row['level_prediction'] == "":
                            ind_results_file = f"results/{llm}/{project}/{shot}/output_finetuned_validate_{project}_{ft_project}_{llm}_{shot}_{row['index']}{suffix}.json"
                    ...
                    df_results[['index', 'project', 'log_level', 'level_prediction']].to_json(f"results/results_{llm}_{project}_{ft_project}_{shot}{suffix}.json", orient="records", indent=2)


def get_fillmask_results_x(suffix):
    ...
    for llm in pretrainedmodels:
        for shot in shots:
            for project in projects:
                ...
                for ft_project in ft_projects:
                    result_file = f"results/output_{project}_{ft_project}_{tokenizer}_{llm}_{shot}{suffix}.txt"
...
def RQ1_baseline():
    df = pd.read_json("log_level_results.json", dtype={'shot': str})
    df = df[df['shot'] == "0.6"]
    df = df[df['llm'].isin(llms)]
    print(df.to_csv(sep='\t', index=False))
def RQ3_cross():
    df = pd.read_json(f"log_level_results_x.json", dtype={'shot': str})
    grouped_df = df.groupby(['llm', 'shot', 'project']).agg({
        'Accuracy': 'mean',
        'AUC': 'mean',
        'AOD': 'mean'
    }).reset_index()

    mean_row = grouped_df[['Accuracy', 'AUC', 'AOD']].mean()

    # Create a new DataFrame for the mean row
    mean_row_df = pd.DataFrame(mean_row).T  # Transpose to make it a single row DataFrame
    mean_row_df['llm'] = 'Mean'  # You can specify a label for the mean row
    mean_row_df['shot'] = ''  # Optionally set this to an empty string or any other label
    mean_row_df['project'] = ''  # Same as above

    # Concatenate the mean row with the original DataFrame
    grouped_df = pd.concat([grouped_df, mean_row_df], ignore_index=True)

    # Round the new mean values to 2 decimal places
...
def RQ3_enlarged():
    df = pd.read_json(f"log_level_results_u.json", dtype={'shot': str})
    grouped_df = df.groupby(['llm', 'shot', 'project']).agg({
        'Accuracy': 'mean',
        'AUC': 'mean',
        'AOD': 'mean'
...
# get_fillmask_results_u("_u")
# get_textgen_results_u("_u")
# calculate_metrics_llm_shot_project_u("_u")

# get_fillmask_results_x("_x")
# get_textgen_results_x("_x")
# calculate_metrics_llm_shot_project_x("_x")

RQ1_baseline() ###Results from Graphcodebert and CodeLlama 7b for comparison
RQ3_cross() ###Results for RQ3 Cross Dataset
RQ3_enlarged() ###Results for RQ3 Enlarged Dataset
