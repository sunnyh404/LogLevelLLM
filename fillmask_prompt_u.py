...
for project in projects:
    ...
    arguments = "_".join([project, model_name.replace("/", ""), os.path.basename(pretrainedmodel_path), str(test_size), "u"])
    ...
    with open(output_file, 'w') as f:    
        ...
        print("test_size: ultimate", file=f)
        print("input: code, constant", file=f)

        if str(test_size) in ("0", "5"):
            train_path = f"logs/{project}_instructions_logs_train_0.6.json"
        else:
            if str(test_size) == "0.5":
                train_path = f"logs/{project}_instructions_logs_train_5.json"
            else:
                train_path = f"logs/{project}_instructions_logs_train_ultimate.json"
            
        if float(test_size) >= 0:
            validate_path = f"logs/{project}_logs_validate_0.6.json"
        else:
            validate_path = f"logs/{project}_logs_validate_{str(test_size)}.json"
